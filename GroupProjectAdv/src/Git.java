...
public class Git {
	...
	public static String TastyVideo(String topicUrl) {

		try {

			Document link = Jsoup.connect(topicUrl).get();

			//			Elements video = link.select("video").select("autoplay src");

			Element video = link.getElementById("vjs_video_3_html5_api");
			...
			System.out.println(video.wholeText());

			return video.wholeText();
